...
async function doLogin(){
  const phone = $('l-phone').value.trim().replace(/\D/g,'');
  if(!phone||phone.length<10){ showErr('Enter your WhatsApp number (e.g. 2348012345678)'); return; }
  const btn=$('l-btn'); btn.textContent='Checking...'; btn.disabled=true;
  try {
    const snap = await db.collection('admin_agents').where('phone','==',phone).get();
    if(snap.empty){ showErr('Phone not registered. Contact AariNAT: +234 814 507 3941'); btn.textContent='▶ Login'; btn.disabled=false; return; }
    const doc = snap.docs[0];
    agent = { id:doc.id, ...doc.data() };
    localStorage.setItem('ag_agent', JSON.stringify(agent));
    startApp();
  } catch(e){ showErr('Connection failed. Check your internet.'); }
